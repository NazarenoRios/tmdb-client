...
export const getUser = createAsyncThunk("GET_USER", ({id,setUser}) => {
    return axios.get(`https://tmdb-api.onrender.com/api/users/user/${id}`,{ withCredentials: true, credentials: 'include' })
      .then(res => setUser(res.data))
})

export const searchUser = createAsyncThunk("SEARCH_USER", ({name,setUsers}) => {
    console.log(name)
    return axios.get(`https://tmdb-api.onrender.com/api/users/search?name=${name}`,{ withCredentials: true, credentials: 'include' })
