...
export const removeFromFavorites = createAsyncThunk("REMOVE_FAVORITE",(movie, thunkAPI) => {
  ...
  if (!users.id) throw new Error("You need to be logged in");
  return axios
    .delete(`/api/movies/removeFavorite?userId=${users.id}&code=${movie.id}`, { withCredentials: true, credentials: 'include' })
    .then((res) => res.data);
}
);
...
export const Favorites = createAsyncThunk("FAVORITES",(setMovies, thunkAPI) => {
  ...
  if (!users.id) throw new Error("You need to be logged in");
  return axios
    .get(`/api/movies/favorites?userId=${users.id}`, { withCredentials: true, credentials: 'include' })
...
export const UserFavorites = createAsyncThunk("USER_FAVORITES",({id, setMovies}) => {
  return axios
    .get(`/api/movies/favorites?userId=${id}`, { withCredentials: true, credentials: 'include' })
    .then((res) => setMovies(res.data));
}
);
